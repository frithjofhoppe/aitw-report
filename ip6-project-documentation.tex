\documentclass[final]{fhnwreport}         %[mode] = draft or final
\input{fhnwheader}                        %loads all packages, definitions and settings	
\input{abbreviations}

%%%%% Logo: Hocvhschule HTU oder HSI, Sprache DE oder EN:
%\newcommand{\logofilename}{FHNW_HTU_DE}
%\newcommand{\logofilename}{FHNW_HTU_EN}
\newcommand{\logofilename}{FHNW_HSI_DE}
%\newcommand{\logofilename}{FHNW_HSI_EN}
%%%%%
%%%%% Bibliographie entweder im IEEE- oder im APA-Stil:
%\usepackage[style=ieee,urldate=comp,backend=biber]{biblatex}
\usepackage[style=apa,urldate=comp,backend=biber,bibencoding=utf8]{biblatex}
%%%%%
% \addbibresource{literature/bib.bib}
\addbibresource{literature/zotero.bib}

\title{Enterprise-Architektur von multicloud-konformen Landingzones}  %Project Title
\author{Bachelorthesis}    %Document Type => Technical Report, ...
\date{Windisch, März 2026}               %Place and Date

\begin{document}

\pagenumbering{roman}	

%%---TITLEPAGE---------------------------------------------------------------------------
\selectlanguage{ngerman}                  %ngerman or english
\maketitle

\vfill

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{bitcloud2.png}
\end{figure}

\vfill

\begin{tabular}{@{}p{5cm} l}
Studentin/Student          &    Frithjof Hoppe\\
                           &    Benjamin Dätwyler\\[2ex]
Expertin/Experte           &    Romano Roth\\[2ex]
Fachbetreuer/in            &    Sebastian Graf\\[2ex]
Auftraggeber               &    Sebastian Matyas\\[2ex]
                           &    Thierry Perroud\\[2ex]
Projektnummer              &    25HS\_IMVS17\\[4ex]
\multicolumn{2}{@{}l}{Fachhochschule Nordwestschweiz, Hochschule für Informatik}
\end{tabular}

\vspace*{4ex}
% Logo Industriepartner
\begin{tikzpicture}[remember picture,overlay,every node/.style={anchor=north east}]
  \node at (current page.north east) [xshift=-1cm, yshift=-1cm] {\includegraphics[width=8cm]{BIT_d_rgb_pos_quer_pf.jpg}};
\end{tikzpicture}

\clearpage

%%---ABSTRACT----------------------------------------------------------------------------
\selectlanguage{ngerman}				%ngerman or english
\thispagestyle{empty}
\include{sections/abstract}

%%---TABLE OF CONTENTS-------------------------------------------------------------------	
\selectlanguage{ngerman}				%ngerman or english
\tableofcontents
\clearpage

\sloppypar
\listoffigures
\endsloppypar
\listoftables
\printglossary[type=\acronymtype]
\cleardoublepage

%%---TEXT--------------------------------------------------------------------------------

\glsresetall % Ignore abbreviate in the list of figures or tables
\pagenumbering{arabic}
\include{sections/einleitung}
\include{sections/grundlagen}
\include{sections/methodik}
\include{sections/methodik_evaluation}
\include{sections/methodik_poc}
\include{sections/traceability-architecture}
\include{sections/umsetzung}
\include{sections/evaluation}
\include{sections/diskussion}
\include{sections/abschluss}
\include{sections/hilfsmittel}

%%---BIBLIOGRAPHY------------------------------------------------------------------------

{\sloppypar
\printbibliography[heading=bibintoc, title=Quellenverzeichnis]
}
\clearpage

%%---APPENDIX----------------------------------------------------------------------------
\include{sections/eigenstaendigkeit}
\selectlanguage{ngerman}				%ngerman or english
% \include{sections/authenticity}
% \selectlanguage{enlish}				%ngerman or english
\include{sections/appendix}

%%---NOTES for DEBUG---------------------------------------------------------------------
%\newpage
%\listoftodos[\section{Todo-Notes}]
%\clearpage

\end{document}