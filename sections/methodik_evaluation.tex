...
Für die Bewertung der \hl{(Ab-)Satz fertig schreiben)}
...
\textbf{Begründung}: Die Nachvollziehbarkeit und das Enforcement von Regulationen ist einer der Beweggründe im Use-Case für die Traceability da
...
\textbf{Begründung}: Traceability selbst ist beriets ein abstrakter Begriff. Ein Metamodell welche ein Mapping auf Cloud-Plattform spezifische Namings / Konzepte erlaubt ist deshalb relevant.
...
Sicherheitsanforderungen an Software und Hardware werden bereits in der Entwicklung berücksichtigt, um spätere Schwachstellen in der Sicherheit zu verhindern.
...
\textbf{Begründung}: Weil es sich bei der Traceability-Architektur selbst um ein Werkzeug für Audits der Foundation handelt, sind Sicherheitsüberlegung noch wichtiger
\textbf{Quelle}: Auszug aus \autoref{fig:bit-architecture-principles}
...
\textbf{Begründung}: Die Annahme auf welcher die Traceability-Architektur strukturiert sowie weitere Konzepte erarbeitet werden sollte plattform-übergreifend anwendbar sein
...
\hl{Entfernen der bewertung der trade off kritierien, da dies in der evaluatio nur textuell erfolgt und nicht mit der skala wie sie hier beschrieben wird}
...
\begin{longtable}{|
  ...
  L{3cm}|
    ...
    Die Fähigkeit der Automatisierung ist einer der Hauptgründe für die Relevanz der Traceability & \hl{Quellen ergänzen} Beschreibung des Kontextes
    \\
    \hline
