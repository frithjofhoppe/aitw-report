...
Mittels der verschiedenen Dimensionen wird ein Evaluationsprozess definiert. Der Evaluationsprozesse wendet die verschiedenen Dimensionen an um einen bewertenden Output generieren zu können. Durch den Output welcher durch Anwendung des Prozesses auf die verschiedenen Traceability-Architekturen soll eine Vergleichbarkeit ermöglicht werden. Schlussendlich kann somit eine Empfehlung im Anwendungskontext gegeben werden. Gedanklich orientiert sich dieser Prozess somit an einer \ac{MCDA} in der verschiedene Konfliktbereiche zueinander ins Verhältnis gesetzt werden.

Der Output des Prozesses ist der $ Score_{total} $. Dieser Score ergibt sich aus teils quantitativen wie auch qualitativen Scores, weshalb er hier als semi-quantitativ bezeichnet wird. Als Score wird hierbei eine bewertende Grösse verstanden.:
\begin{itemize}
    \item $Score_{principles}$ als quantitativer Score basierend auf den Architekturprinzipien
    \item $Score_{quality}$ als quantitativer Score basierend auf den Qualitätsmerkmalen
    \item $Score_{tradeoff}$ als qualitativer Score basierend auf den Trade-Off Kriterien
\end{itemize}

\begin{align}
Score_{principles} &= \sum_{i=1}^{n} w_i \cdot p_i \\
Score_{quality} &= \sum_{j=1}^{m} v_j \cdot q_j \\
\end{align}

\[
Score_{total} = Score_{principles} + Score_{quality} + Score_{tradeoff}
\]
